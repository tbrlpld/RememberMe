...
function runAfterDOMIsBuild(){
	...
	for (var row = 1; row <= cardsPerRow; row++) {
		for (var column = 1; column <= cardsPerColumn; column++) {
			...
			$(".card-area").append('<div class="memory-card"><div class="card-content">content</div></div>');
			lastCardCreated = $('.memory-card').last();
			lastCardCreated.css('grid-row', String(row));
			lastCardCreated.css('grid-column', String(column));
			lastCardCreated.css('background-image', 'url("https://icon-icons.com/icons2/614/PNG/512/wifi-symbol-inside-a-circle_icon-icons.com_56445.png")');
			lastCardCreated.css('background-size', 'cover');
		}
	}
}
